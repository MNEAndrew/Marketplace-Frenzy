...
export function BuildPanel({ coins, slots, selectedSlotId, onBuild }) {
  ...
  const emptySelected = selectedSlotId && selected && !selected.built;
          ...
          return (
            <li key={typeId}>
              <button
                ...
                disabled={disabled}
                onClick={() => emptySelected && onBuild(selectedSlotId, typeId)}
              >
                <span className="build-row__emoji" aria-hidden="true">
                  {def.emoji}
                </span>
